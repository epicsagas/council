...
pub async fn handle_save_summary(params: Value) -> Result<Value> {
    ...
    // Debug logging
    eprintln!(
        "DEBUG: save_summary called with params: title={}, model={}, content_len={}",
        title,
        model,
        summary_content.len()
    );
    ...
    fs::write(&summary_md_path, &markdown).context(format!(
        "Failed to write summary file: {} (searched from: {})",
