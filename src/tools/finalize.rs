...
pub async fn handle_finalize(params: Value) -> Result<Value> {
    ...
    let engine = params["engine"].as_str().unwrap_or(model);
    ...
    // Debug logging
    eprintln!(
        "DEBUG: finalize called with params: title={}, model={}, council_base={}, base_dir={}",
        title,
        model,
    ...
    for file_path in &answer_files {
        let parsed = read_stage1_answer(file_path).context(format!(
    ...
    for file_path in &review_files {
        let parsed = read_stage2_review(file_path).context(format!(
